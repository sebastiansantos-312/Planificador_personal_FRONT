        ...
        <div className="min-h-screen bg-slate-950 flex flex-col">
            ...
            <header className="bg-slate-900 border-b border-slate-800 sticky top-0 z-40">
                <div className="max-w-3xl mx-auto px-4 h-16 flex items-center justify-between">
                    ...
                    <nav className="flex items-center gap-1.5" role="navigation" aria-label="Navegación principal">
                        {navItems.map(({ to, label, icon }) => (
                            <NavLink
                                key={to}
                                to={to}
                                className={({ isActive }) =>
                                    `flex items-center gap-2 px-3.5 py-2 rounded-xl text-sm font-medium transition-all duration-150 focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 ${isActive
